...
main( int argc, char *argv[ ] )
{
	...
	for( int n = 0;  n < MAXITERATIONS; n++ )
	{
        ...
        #pragma omp parralel for default(none) shared(Cities, Capitals)
		for( int i = 0; i < NUMCITIES; i++ )
		{
			int capitalnumber = -1;
			...
			for( int k = 0; k < NUMCAPITALS; k++ )
			{
				...
				if( dist < mindistance )
				{
					capitalnumber = k; //find number 
                    mindistance = dist;//set new minimum distance
				}
			}
            Cities[i].capitalnumber = capitalnumber;//update the capital number
...
#else
        fprintf(stderr, "%2d threads : %4ld cities ; %4d capitals; megatrials/sec = %8.3lf\n",
                NUMT, NUMCITIES, NUMCAPITALS, megaCityCapitalsPerSecond );
#endif
